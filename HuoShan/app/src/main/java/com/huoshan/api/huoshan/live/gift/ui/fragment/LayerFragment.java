...
import com.huoshan.api.huoshan.live.gift.tools.DisplayUtil;
import com.huoshan.api.huoshan.live.gift.tools.SoftKeyBoardListener;
import com.huoshan.api.huoshan.live.gift.view.CustomRoundView;
import com.huoshan.api.huoshan.live.gift.view.HorizontalListView;
import com.huoshan.api.huoshan.live.gift.view.MagicTextView;
...
public class LayerFragment extends Fragment implements View.OnClickListener {
   ...
   //烟花
    private void showGift4(String tag) {
        ...
        if (giftView == null) {/*该用户不在礼物显示列表*/
            ...
            giftView = addGiftView4();/*获取礼物的View的布局*/
    ...
    //添加礼物view,(考虑垃圾回收) 烟花
    private View addGiftView4(){
        View view = null;
        if (giftViewCollection.size() <= 0) {
            /*如果垃圾回收中没有view,则生成一个*/
            view = LayoutInflater.from(getActivity()).inflate(R.layout.item_gift4, null);
   ...
   //火箭
    private void showGift3(String tag) {
        View giftView = llgiftcontent.findViewWithTag(tag);
        if (giftView == null) {/*该用户不在礼物显示列表*/
            ...
            giftView = addGiftView3();/*获取礼物的View的布局*/
            giftView.setTag(tag);/*设置view标识*/
    ...
    //添加礼物view,(考虑垃圾回收) 火箭
    private View addGiftView3(){
        View view = null;
        if (giftViewCollection.size() <= 0) {
            /*如果垃圾回收中没有view,则生成一个*/
            view = LayoutInflater.from(getActivity()).inflate(R.layout.item_gift3, null);
            LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
            lp.topMargin = 10;
            view.setLayoutParams(lp);
            llgiftcontent.addOnAttachStateChangeListener(new View.OnAttachStateChangeListener() {
    ...
    //飞机
    private void showGift2(String tag) {
        View giftView = llgiftcontent.findViewWithTag(tag);
        if (giftView == null) {/*该用户不在礼物显示列表*/
            ...
            giftView = addGiftView2();/*获取礼物的View的布局*/
            giftView.setTag(tag);/*设置view标识*/
